...
public final class MainExists {
     ...
     */
    public static void ensureMainExists(Location loc, ArrayList<AST.Declaration<Unit, Types.Parsed, String>> declarations) throws CompilerError {
        boolean found = false;
        for (AST.Declaration<Unit, Types.Parsed, String> d: declarations) {
            
            if (d.tag == AST.DeclarationTag.Def) {
                AST.Def<Unit, Types.Parsed, String> def = (AST.Def<Unit, Types.Parsed, String>)d;
                ...
                if (def.name.equals("main")) {
                    ...
                    ArrayList<AST.Reference<Types.Parsed, String>> args = def.arguments;
                    if (args.size() != 1) {
